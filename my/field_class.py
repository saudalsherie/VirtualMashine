...
def get_sheep_name():
  ...
  def grow(self,light,food,water):
    ...
    if len(self._animals) > 0:
      #grow the animals (water available to all animals in same amount
      #but food is a total that must be shared
      food_required = 0
      #get a total of food required by the animals in the field
      for animal in self._animals:
        needs = animal.needs()
        food_required += needs["food need"]
      #if we have more food available than is required,
      #work out the additional food available
        ...
        for animal in self._animals:
          ...
          if food >= needs["food need"]:
            ...
            if additional_food > 0:
              #remove some additionalfood for this animal
              addition_food -= 1
              #add additional food to the feed
              feed +=1
            #grow animal
            animal.grow(feed,water)

def auto_grow(field,days):
  #grow field automatically
...
def display_crop_menu():
  ...
  print()
  print("Please select and option from the above menu")
...
def display_main_menu():
  ...
  print()
  print("7. Report the field status")
  print()
...
def manage_field(field):
  ...
  while not exit:
    ...
    if option == 1:
      ...
      print("You have successfully removed a {0} crop".format(removed_crop))
    elif option == 3:
      add_animal_to_field(field)
    elif option == 4:
      remove_animal = remove_animal_from_field(field)
      print("You have successfully removed a {0}".format(removed_crop))
    elif option == 5:
      manual_grow(field)
    elif option == 6:
      auto_grow(field,30)
